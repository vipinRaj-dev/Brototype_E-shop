...
const signup_post = async (req, res) => {
    try {
        ...
        if(Referal){
            ...
            await usercollection.findOneAndUpdate({'referalCode.code':Referal,'referalCode.count':{$lte:5}},{$inc:{walletbalance:100,'referalCode.count':1}})
...
const login_OTPValidation = async (req, res) => {
    ...
    try {
        ...
        await otpcollection.find({ number: enteredOtp })
        .then((found) => {
                let entrie = 0;
                if (found.length > 0) {
                    ...
                    otpcollection.findOneAndDelete({ number: enteredOtp })
                        .then(() => {
                    ...
                    res.render('user/validation', { msg: "Wrong OTP Re-enter the OTP", user: req.session.user, cartCount,entrie })
                }
            })
            .catch((err) => {
                let entrie = 0;
                console.log('an error occured while fetching the otp from database in login post',err);
...
const home = async (req, res) => {
    try {
        ...
        const products = await productCollection.find({$and: [{ availability: true }, { softdelete: true }]}).limit(10);
        const product = await productCollection.find({$and : [{ availability: true} ,{softdelete :true}]}).skip(10).limit(10)
...
const productView = async (req, res) => {
    ...
    try {   
        ...
        res.render('user/product-view', { data, user: req.session.user,price,cartCount,name})
    } catch (error) {
  ...
  const filter = async(req, res) => {
    ...
    const Addcart = async (req, res) => {
        try {
            ...
            const cartItemExists = cartItems.find(item => item.productId.toString() === id);
