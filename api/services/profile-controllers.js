...
const listReturn = async (req, res) => {
    try {
        ...
        const totalAmount = returnProduct.filter((product)=>product.status ==='Return Accepted').reduce((sum, item) => sum + item.payment.amount, 0);
        ...
        const order = await ordercollection.updateMany({ status: 'Return Accepted', userId: userid },
