...
const dashboard = async(req,res)=>{
    try{
        if(req.session.admin){
            ...
            const refundOrders = order.filter((data) => data.status === "Refund Done");
            const ProcessingOrders = order.filter((data) => data.status === "Processing");
            const orderCanceled = order.filter((data) => data.status === "Canceled");
            const returnRequested = order.filter((data)=>data.status === "Return Requested")
...
const login_post = async (req, res) => {
    try {
      ...
      res.render('admin/adminlogin', { msg: 'Invalid login credentials', admin: true });
    } catch (error) {
...
const create_category_post = async (req, res) => {
    ...
    categoryName: {
        ...
        if (category) {
            ...
            const admin = false;
            res.render('admin/category-create', { errmsg: 'Category already exist', admin });
        } else {
...
const add_product_post = async (req, res, next) => {
    try {
         ...
         const updatingPrice = Math.round(originalprice-((originalprice*productOffer)/100))
...
const productDelete = async (req, res) => {
    try {
        ...
        await productCollection.findByIdAndUpdate({_id:id},{$set:{softdelete:false}})
        res.redirect('/admin/view-product');
    } catch (error) {
...
const orderDetails = async (req, res) => {
    try {
        ...
        let subTotal = 0, discount, totalPrice = 0;
        ...
        res.render("admin/orderDetails", { admin, order:[order], orderProducts, subTotal, address, orderCanceld, orderStatus, userDetails , discount, totalPrice});
    } catch (error) {
